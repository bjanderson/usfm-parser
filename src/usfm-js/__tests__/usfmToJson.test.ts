...
describe('USFM to JSON', () => {
  ...
  it('handles Heb 1:1 alignment - oldformat', () => {
    generateTest(
      'heb1-1_multi_alignment.oldformat',
      { convertToInt: ['occurrence', 'occurrences'], map: { ugnt: 'content' }, oldFormat: true },
  ...
  it('handles mat-4-6.whitespace.oldformat', () => {
    generateTest(
      'mat-4-6.whitespace.oldformat',
      { zaln: true, oldFormat: true },
      'mat-4-6.whitespace',
    );
  });
  ...
  it('process usfm-body-testF-paragraph-whitespace', () => {
    generateTest(
      'usfm-body-testF-paragraph-whitespace',
  ...
  it('process exported alignment acts_1_11', () => {
    generateTest('acts_1_11.aligned', { convertToInt: ['occurrence', 'occurrences'], words: true });
  });

  it('process exported alignment acts_1_11 - oldformat', () => {
    generateTest(
      'acts_1_11.aligned.oldformat',
  ...
  it('process exported 57-TIT.greek', () => {
    generateTest('57-TIT.greek', { convertToInt: ['occurrence', 'occurrences'], words: true });
  });
  ...
  it('process exported 57-TIT.partial', () => {
    generateTest('57-TIT.partial', { convertToInt: ['occurrence', 'occurrences'], words: true });
  });
 ...
 */
const generateTest = (name, args: any = {}, expectedName = '') => {
  ...
  if (args.oldFormat) {
    if (output.headers) {
      ...
      if (pos < 0) {
        // put in usfm 3 flag for comparison if not present
  ...
  if (JSON.stringify(output) !== JSON.stringify(expected)) {
    // if different, break down comparisons by smaller chunks so output is not overloaded
    ...
    if (expected[textField]) {
      verifyWithPrompt(
        Object.keys(output[textField]),
        Object.keys(expected[textField]),
        'chapter numbers',
      );
      if (JSON.stringify(output[textField]) !== JSON.stringify(expected[textField])) {
        console.log('Chapter data is different');
        const chapters = Object.keys(output[textField]);
        chapters.forEach((chapter) => {
          verifyWithPrompt(
            Object.keys(output[textField][chapter]).sort(),
            Object.keys(expected[textField][chapter]).sort(),
            'verses',
          );
          const verses = Object.keys(output[textField][chapter]);
          verses.forEach((verse) => {
            verifyWithPrompt(
              Object.keys(output[textField][chapter][verse]),
              Object.keys(expected[textField][chapter][verse]),
              'verse ' + chapter + ':' + verse,
            );
          });
          verifyWithPrompt(
            output[textField][chapter],
            expected[textField][chapter],
            'chapter ' + chapter,
          );
        });
      }
