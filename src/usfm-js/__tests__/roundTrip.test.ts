...
function keyRightTrim(received) {
  if (
    received.indexOf('\\k-s') < 0 &&
    received.indexOf('\\zaln-s') < 0 &&
    received.indexOf('\\w') < 0
  ) {
    return received;
  }
...
function validateUSFM(usfm, input) {
  ...
  if (failed) {
    ...
    const lines = source.length < results.length ? source.length : results.length;
