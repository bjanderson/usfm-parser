import { removeMarker } from '../filter';
import { readUSFM } from './util';

const input =
  "...Christ\\q Jesus. \\f + \\ft Some early versions omit, \\fqa in Ephesus, \\fqa* but this expression is probably in Paul's original letter.\\f*";
...
it('removes f tags and following s5 tag', () => {
  const input =
    'He even tried to desecrate the temple, so we arrested him.\\f + \\ft Some ancient copies add, \\fqa "We wanted to judge him according to our law \\fqa* . \\f*\\s5';
...
it('removes f tags and following p tag', () => {
  const input =
    '\\f + \\ft Acts 28:29—Some ancient copies have verse 29: \\fqa When he had said these things, the Jews went away. They were having a great dispute among themselves \\fqa* . \\f*\\p\n';
  const expected = '\n';
  const output = removeMarker(input);
  expect(output).toEqual(expected);
});
