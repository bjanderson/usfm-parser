...
const parseLine = (line, lastLine) => {
  ...
  if (regex.exec(line)) {
    ...
    for (let i = 0, len = matches.length; i < len; i++) {
      ...
      const marker: any = parseMarkerOpen(open);
      let object: any = {
        open: open,
        tag: marker.tag,
...
const addHeaderMarker = (state, marker) => {
  ...
  if (appendToLast) {
    ...
    let content =
      (lastHeader[key] || '') + (lastHeader.next || '') + markerToText(marker, noSpaceAfterTag);
    ...
    switch (marker.tag) {
      ...
      case 'zaln': {
        ...
        if (state.inHeader) {
          ...
          const phrase: any = parseWord(state, marker.content); // very similar to word marker, so start with this and modify
