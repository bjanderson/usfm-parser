 ...
 */
const usfmMarkerToString = (usfmObject, nextObject = null, noSpaceAfterTag = false, noTermination = false) => {
    let output = '';
    let content = usfmObject.text || usfmObject.content || '';
    let markerTermination = usfmObject.endTag; // new format takes precidence
    if (typeof markerTermination !== 'string' &&
        USFM.markerTermination(usfmObject.tag) &&
        !noTermination) {
    ...
    if (usfmObject.tag) {
        ...
        if (noSpaceAfterTag) {
            ...
            if (firstChar !== '' && firstChar !== '\n' && content !== ' \n') {
                ...
                usfmObject.tag &&
                !content && // make sure some whitespace
                !usfmObject.nextChar &&
...
const objectToString = (object, output, nextObject = null) => {
    ...
    if (object.type === 'text') {
        ...
        if (hasParagraph(text)) {
            ...
            const verseObjects = (0, usfmToJson_1.usfmToJSON)('\\v 1 ' + text, { chunk: true });
            const newText = (0, exports.jsonToUSFM)(verseObjects).substr(5); // convert back to text and string out verse marker
            if (newText !== text) {
...
const jsonToUSFM = (json, params) => {
    ...
    if (json.chapters) {
        ...
        for (let i = 0; i < chapterLen; i++) {
            ...
            const chapterLines = generateChapterLines(chapterNumber, chapterObject);
    ...
    if (json.verses) {
        ...
        for (let i = 0; i < verseLen; i++) {
            ...
            const verse = generateVerse(verseNumber, verseObjects);
