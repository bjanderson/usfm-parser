...
const pushObject = (state, saveTo, usfmObject) => {
    ...
    if (typeof usfmObject === 'string') {
        ...
        usfmObject = (0, exports.createUsfmObject)(usfmObject);
    }
...
const crammedMarkers = (lastObject, currentTag) => {
    const crammed = lastObject && isAlignmentMarker(lastObject) && lastObject.tag === currentTag;
    return crammed;
};
/**
        ...
        if (lastObject.nextChar === '\n') {
            ...
            if (isLastCharNewLine(text)) {
                if (text.length === 1) {
                    ...
                    if (!state.newAlignFormat &&
                        lastObject.text === ' ' &&
                        USFM.markerDisplayable(lastObject.tag)) {
                        lastObject.nextChar = lastObject.text;
                        delete lastObject.text;
                    }
                }
                replaceWithSpace =
                    state.newAlignFormat || (!endTag && crammedMarkers(lastObject, currentTag));
            }
        }
    }
    if (replaceWithSpace) {
        (0, exports.pushObject)(state, null, ' ');
    }
};
/**
 * @description - remove previous new line from text
...
const handleWordWhiteSpace = (state) => {
    ...
    if (saveTo && saveTo.length) {
        ...
        if (lastObject.nextChar === '\n') {
            ...
            const text = lastObject.text;
            if (isLastCharNewLine(text)) {
                const startOfLine = saveTo.length === 1 && lastObject.text.length === 1;
...
const saveUsfmObject = (state, marker) => {
    ...
    if (phraseParent) {
        if (!USFM.markerContentDisplayable(phraseParent.tag)) {
            ...
            const usfmObject_ = (0, exports.createUsfmObject)(marker);
            saveTo.push(usfmObject_);
        }
    }
    else if (state.nested.length > 0) {
        // is nested object
        (0, exports.pushObject)(state, null, marker);
    }
    else {
        // not nested
        const saveTo = getSaveToLocation(state);
            ...
            if (termination) {
                matchesParent =
                    termination === endTagBase ||
                        ...
                        phraseParent.tag.substr(0, phraseParent.tag.length - 2) + termination + '\\*' ===
                            endTagBase;
                if (matchesParent) {
                    break;
                }
...
const endSpan = (state, index, markers, endMarker, header = false) => {
    ...
    const phraseParentProps = (phraseParent && USFM.USFM_PROPERTIES[phraseParent.tag]) || null;
    ...
    if (phraseParent) {
        ...
        const { matchesParent, count } = decrementPhraseNesting(state, phraseParent, endMarker);
        ...
        if (!parentContentDisplayable) {
            ...
            if (!finishPhrase) {
                phraseParent.content = (phraseParent.content || '') + endMarker_ + nextChar;
            }
            else {
                phraseParent.endTag = endMarker;
                if (nextChar) {
    ...
    if (content || !phraseParent) {
        saveUsfmObject(state, (0, exports.createUsfmObject)(content, header));
    }
    return index;
};
/**
...
const addToCurrentVerse = (state, marker) => {
    const tag = marker.tag;
    if (!tag) {
        (0, exports.pushObject)(state, null, (0, exports.createUsfmObject)(marker));
        return;
    }
    saveUsfmObject(state, (0, exports.createUsfmObject)(marker));
};
/**
...
const parseAsVerse = (state, marker) => {
    ...
    if (state.params.chunk && !state.onSameChapter) {
        if (state.verses[state.currentVerse]) {
            ...
            state.verses[state.currentVerse] = [];
            (0, exports.pushObject)(state, null, marker.content);
        }
    }
...
const processAsText = (state, marker) => {
    if (state.currentChapter > 0 && marker.content) {
        ...
        (0, exports.pushObject)(state, state.headers, (0, exports.createUsfmObject)(marker));
    }
    if (state.params.chunk && state.currentVerse > 0 && marker.content) {
        ...
        if (getPhraseParent(state)) {
            ...
            (0, exports.pushObject)(state, state.verses[state.currentVerse], marker.content);
        }
    }
};
...
const addHeaderMarker = (state, marker) => {
    ...
    const usfmObject = (0, exports.createUsfmObject)(marker, true);
    if (nextChar !== undefined) {
        usfmObject.nextChar = nextChar; // add back in
    }
    const lastHeader = state.headers.length ? state.headers[state.headers.length - 1] : null;
...
const processMarkerForSpans = (state, marker, i, markers) => {
    ...
    if (endMarker) {
        ...
        i = startSpan(state, (0, exports.createUsfmObject)(marker), marker.tag, i, markers);
    }
    else {
    ...
    for (let i = 0; i < state.headers.length; i++) {
        ...
        if (header.type === 'text') {
            ...
            while (i + 1 < state.headers.length) {
                const headerNext = i + 1 < state.headers.length ? state.headers[i + 1] : {};
 ...
 * @param {object} state - holds parsing state information
 */
const getAlignmentFormat = (usfm, state) => {
...
const usfmToJSON = (usfm, params = {}) => {
    ...
    (0, exports.getAlignmentFormat)(usfm, state);
        ...
        switch (marker.tag) {
            ...
            case 'w': {
                ...
                if (state.inHeader) {
                    ...
                    const wordObject = parseWord(state, marker.content, USFM.wordSpecialAttributes);
                    if (isNonDisplayablePhraseParent(state)) {
                        saveUsfmObject(state, marker);
                    }
                    else {
                        (0, exports.pushObject)(state, null, wordObject);
                        if (marker.nextChar) {
                            (0, exports.pushObject)(state, null, marker.nextChar);
                        }
                    }
                }
                break;
